...
save_dir = os.path.join(rootdir, "D2V/extracted")
...
dataset = Dataset_OpenML(args.dataset)
...
pd.DataFrame(metafeatures, index=[dataset.name]).to_csv(os.path.join(save_dir, f"{dataset.name}.csv"))
